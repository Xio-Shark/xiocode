...
describe("formatExploreResult / prompts", () => {
  it("names subagents by purpose with role/goal fallback", () => {
    expect(parseExploreName("  Auth   Flow  ", undefined, "goal")).toBe("Auth Flow");
    expect(parseExploreName(undefined, "flow_analyst", "goal")).toBe("flow analyst");
    expect(parseExploreName(undefined, undefined, "survey the top-level structure fully"))
      .toBe("survey the top-level structure");
    expect(parseExploreName("", undefined, "")).toBe("explore");
    expect(parseExploreName("x".repeat(80), undefined, "goal")).toHaveLength(40);
  });
...
describe("Phase 0 characterization (ADR 0003 gate — no behavior change)", () => {
  it("explore tool schema omits provider/model/reasoning overrides", () => {
    ...
    expect(keys).toEqual(["focus_paths", "goal", "max_turns", "name", "role"]);
